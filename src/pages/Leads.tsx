...
function Leads() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir seu lead?')) {
      try {
        // Este params está vindo do AxiosRequestConfig
        await leadsDeleteData({ params: { id: id } })
        alert('Lead deletado com sucesso!')
        // Este getLeads é para atualizar a tela e mostrar que o lead foi apagado
        getLeads()
      } catch (e) {
        alert(
  ...
  return (
    <>
      <Header />
      <Container className="mb-2" maxWidth="lg"></Container>
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={5}>
          <CardComponent>
            <StyledH2 className="mb-1">Cadastrar leads leads</StyledH2>
            <FormComponent
              ...
              buttons={[
                {
                  className: 'primary',
                  // Abaixo mostra as condições para que o botão fique desabilitado
                  disabled:
